# -*- coding: utf-8 -*-
# 橘汁视频 TVBox spider — 首页去掉推荐，打开直接剧集，移除直播分类
import json
import re
import sys
import time
...
class Spider(_Base):
    ...
    def homeContent(self, f):
        ...
        try:
            ...
            raw_list = json.loads(d).get('data') or []
            # 只允许这6个，彻底屏蔽直播
            allow_names = ["剧集", "电影", "综艺", "动漫", "短剧", "漫剧"]
            order_map = {
                "剧集":0,
                "电影":1,
                "综艺":2,
            ...
            sorted_list = sorted([c for c in raw_list if c.get("name","") in allow_names],
                                 key=lambda x: order_map.get(x.get("name",""),999))
    ...
    def homeVideoContent(self):
        # 首页直接剧集列表，不要首页推荐
        return {'list': self._list('page=1&pagesize=24&typeId1=22')}
